...
**WIZ as target (2026-08-31)** (`phase_b_wiz.py`; `forecasts/phase_b_wiz/ablation_WIZ.csv`, `nested_WIZ.csv`): full 15-subset landscape over {FWVZ,KRVZ,ONTA,SHW} (11 pools reused from the FWVZ/KRVZ runs, 4 trained: full/FKO/FKS/FK). **Whakaari is the easiest target**: best pool {ONTA,SHW} 0.9708, {FWVZ} 0.9678, {KRVZ,ONTA,SHW} 0.9598 vs full 0.8787; worst {KRVZ} 0.7301. Nested 5-fold selection: **0.9551** (oracle 0.9708, full 0.8787), {ONTA,SHW} selected 4/5 folds; the held-out Dec-2019 (fatal) eruption scores 0.9843 nested vs 0.7732 full-pool. Key asymmetry: WIZ data poisons other targets' pools, yet WIZ itself is forecast superbly from foreign data — its precursors are cleanly ergodic, its background regime is the contaminant. **{ONTA,SHW} convergence**: the same 2-source foreign pool is now top/near-top for all three targets (WIZ 0.971, KRVZ 0.885, FWVZ 0.957).

**External validation on 8 out-of-pool volcanoes (2026-08-31)** (`phase_b_external.py`; `forecasts/phase_b_external/external_validation.csv`): trained ensembles applied unchanged to PN7A/PVV (Pavlof), VNSS (Veniaminof), BELO (Bezymianny), COP (Copahue), MBGH (Montserrat), VRLE, VTUN — 31 eruptions never seen by any ensemble. **The curation rules generalize**: mean AUC across the 8 targets — {ONTA,SHW} 0.785 and {KRVZ,ONTA,SHW} 0.781 vs the all-5-source generalized model 0.630 (worst of all pools tested). **{KRVZ,ONTA,SHW} beats the all-5 model on 8/8 targets** (sign-test p≈0.004). Matched WIZ-pairs: dropping WIZ from {W,K,O,S} helps at **8/8 targets** (mean +0.12); from {W,F,O,S} it is mixed (2/8, mean +0.02) — the Whakaari harm is robust in the KRVZ-containing family, noisier alongside FWVZ. Absolute skill varies by volcano: COP 0.95, BELO 0.93, VTUN 0.90, MBGH 0.86 forecast well; VRLE (≤0.64) and VNSS (≤0.67) poorly — style-dependence to examine in Phase H.
...
**WIZ as target (2026-08-31)** (`phase_c_wiz.py`; `forecasts/phase_c/phase_c_results_WIZ.csv`): with 20 positive windows from 5 eruptions, **every refinement method beats the full-pool base** (0.8787): strut 0.9344, ser 0.9318, mix 0.9286, strut_us 0.9533, **ser_us 0.9579**. The Phase C failure at FWVZ/KRVZ is a data-scarcity effect, not a method failure — refinement crosses from harmful to helpful somewhere between 2-3 and 5 target eruptions, and ser_us on the unfiltered full pool (0.958) matches nested pool curation (0.955) without any pool search. Diversity preservation (_us) still adds ~+0.02 on top even in the data-rich regime.

**Background-only (label-free) adaptation (2026-08-31)** (`phase_c_bgonly.py`; `forecasts/phase_c_bgonly/bgonly_results.csv`): adaptation fitted on eruption-free background only (all eruptions ±1 month excluded — the "no recorded eruptions" deployment case), evaluated on the real eruptions. Methods: bg_strut (thresholds re-anchored to target-background quantiles matching source routing proportions) and qmap (feature quantile mapping to the source distribution); both fit in seconds. Per-target vs full-pool base: FWVZ 0.845→0.864/0.858, WIZ 0.879→0.901/0.902, KRVZ 0.649→0.584/0.596 (hurts). Pooled 3-volcano AUC (one shared alert threshold): raw base 0.788 → **per-station background-percentile calibration alone 0.821** (+0.033, no model change); calibrated qmap best at 0.826. **Verdict**: threshold calibration to the local noise floor is free, safe, and material for multi-volcano operations; label-free model adaptation gives small gains at 2/3 targets but is not a substitute for pool curation (+0.10-0.24). Caveat: adaptation uses the whole eruption-free record (not pseudo-prospective).
...
## Phase D Progress (2026-08-31)

`puia/fine_tuning.py:TransferBoostEnsemble` (multi-source TrAdaBoost, 40 iters, depth-6 weak learners, class-balanced domain-equal init) + `phase_d_local.py` (Phase-C-mirror LOO protocol; 1296 complete-case common features). Results (`forecasts/phase_d/phase_d_results.csv`):

**Diagnostics (uniform across all 13 fits)**: (1) classic TrAdaBoost **weight collapse** — final target weight fraction ≈ 1.00, source pool decayed to nothing: the method degenerates to target-dominated AdaBoost, it does NOT do source transfer; (2) **instance weighting is blind to the Whakaari harm** — within residual source weight WIZ retains ~50% wherever present: WIZ's individual windows are easy to classify (loud/separable); its harm acts through feature selection (see Phase H below), invisible at instance level.

**Apparent skill**: tboost AUC 0.9965 (FWVZ), 0.9984 (KRVZ), 0.9988 (WIZ) vs bases 0.845/0.649/0.879. Two artifacts were checked: a strict out-of-sample eval using only each fold's excluded month (never in that model's training) still gives 0.997/0.9995/0.997 vs base 0.869/0.671/0.812 on identical windows; and the models are near-silent on quiet years (background p95 consensus = 0.000). **Caveats before believing it**: (a) within-volcano LOO folds contain sister eruptions of the same unrest sequence (KRVZ's two events are 3 months apart) — detecting one from the other ≠ forecasting the next episode; (b) the razor-sharp background silence may be volcano-background regime memorisation, untested against regime change; (c) this is effectively supervised single-volcano learning, a different regime from the leave-volcano-out bases. **Phase G's pseudo-prospective (train-past/forecast-future) evaluation is the arbiter** — if tboost survives that, it's the headline method for targets with any recorded eruptions.
